...
from .constants import (
    ...
    CONV_1E9,
    LENGTH_THRESHOLD_KM,
    LENGTH_THRESHOLD_MICRO,
    LENGTH_THRESHOLD_MICRO_M,
    LENGTH_THRESHOLD_MM,
    LENGTH_THRESHOLD_NANO,
    ZERO_ENERGY_TOL,
)
...
def format_with_units(value: float, units: str, quantity_type: str) -> str:
    ...
    if quantity_type == "length":
        if units == "SI":
            if abs_val < LENGTH_THRESHOLD_MM:
...
def format_value_with_units(value: float, unit_type: str = "length") -> str:
    ...
    if unit_type == "length":
        if abs_val < LENGTH_THRESHOLD_MICRO:
            return f"{value * 1e12:.3f} pm"
        elif abs_val < LENGTH_THRESHOLD_NANO:
            return f"{value * 1e9:.3f} nm"
        elif abs_val < LENGTH_THRESHOLD_MICRO_M:
            return f"{value * 1e6:.3f} μm"
        elif abs_val < 1:
            return f"{value * 1e3:.3f} mm"
        elif abs_val < CONV_1E3:
            return f"{value:.3f} m"
        else:
            return f"{value:.3f} m"
        ...
        if abs_val < LENGTH_THRESHOLD_MICRO_M:
        ...
        if abs_val < LENGTH_THRESHOLD_NANO:
    ...
    if format in ["csv", "both"]:
        ...
        with open(summary_file, "w", newline="", encoding="utf-8-sig") as f:
            ...
            writer.writerow(
                [
                    "Supported Nodes",
                    report["metadata"]["supported_nodes"],
                    "-",
                    "گره‌های تکیه‌گاهی",
                ]
            )
            ...
            writer.writerow(
                [
                    "Temperature Change",
                    report["metadata"]["global_temperature_change"],
                    "°C",
                    "تغییر دمای سراسری",
                ]
            )
            ...
            writer.writerow(
                [
                    "Total Energy",
                    report["energy_statistics"]["total_energy"],
                    "J",
                    "انرژی کل کرنشی",
                ]
            )
            ...
            if max_force["id"] is not None:
                writer.writerow(
                    [
                        "Max Force Element ID",
                        max_force["id"],
                        "-",
                        "شناسه عضو با بیشترین نیرو",
                    ]
                )
            ...
            writer.writerow(
                [
                    "Tension Elements",
                    report["force_distribution"]["tension_elements"],
                    "-",
                    "تعداد اعضای کششی",
                ]
            )
            writer.writerow(
                [
                    ...
                    report["force_distribution"]["compression_elements"],
                    "-",
                    "تعداد اعضای فشاری",
                ]
            )
            writer.writerow(
                [
                    "Max Tensile Force",
                    report["force_distribution"]["max_tensile_force"],
                    "N",
                    "بیشترین نیروی کششی",
                ]
            )
            writer.writerow(
                [
                    ...
                    report["force_distribution"]["max_compressive_force"],
                    "N",
                    "بیشترین نیروی فشاری",
                ]
            )
            ...
            if disp_stats["max_displacement_node"] is not None:
                writer.writerow(
                    [
                        ...
                        disp_stats["max_displacement_node"],
                        "-",
                        "گره با بیشترین جابجایی",
                    ]
                )
            writer.writerow(
                [
                    "Max X Displacement",
                    disp_stats["max_x_displacement"],
                    "m",
                    "بیشترین جابجایی در راستای X",
                ]
            )
            writer.writerow(
                [
                    "Max Y Displacement",
                    disp_stats["max_y_displacement"],
                    "m",
                    "بیشترین جابجایی در راستای Y",
                ]
            )
            writer.writerow(
                [
                    "RMS Displacement",
                    disp_stats["rms_displacement"],
                    "m",
                    "میانگین مربعات جابجایی",
                ]
            )
            ...
            writer.writerow(
                [
                    "Elements with I",
                    buckling_stats["elements_with_I"],
                    "-",
                    "اعضای دارای ممان اینرسی",
                ]
            )
            writer.writerow(
                [
                    "Elements at Risk",
                    buckling_stats.get("elements_at_risk", 0),
                    "-",
                    "اعضای در معرض کمانش",
                ]
            )
            writer.writerow(
                [
                    "Buckling Warnings",
                    buckling_stats.get("warning_count", 0),
                    "-",
                    "تعداد هشدارهای کمانش",
                ]
            )
            writer.writerow(
                [
                    "Max Buckling Ratio",
                    buckling_stats["max_buckling_ratio"],
                    "-",
                    "بیشترین نسبت کمانش",
                ]
            )
            ...
            writer.writerow(
                [
                    "Elements with ΔT ≠ 0",
                    thermal_stats["elements_with_delta_T"],
                    "-",
                    "اعضا با تغییر دمای غیرصفر",
                ]
            )
            writer.writerow(
                [
                    "Elements with δL₀ ≠ 0",
                    thermal_stats["elements_with_delta_L0"],
                    "-",
            ...
            writer.writerow(
                [
                    ...
                    "Yes" if validation_stats["sign_convention_valid"] else "No",
                    "-",
            ...
            writer.writerow(
                [
                    ...
                    "Yes" if validation_stats["energy_balance_valid"] else "No",
                    "-",
                    "اعتبار تعادل انرژی",
                ]
            )
