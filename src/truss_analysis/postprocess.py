...
def calculate_percentages(results: List[Dict]) -> List[Dict]:
    ...
    # بررسی شرایط ویژه
    if not np.isfinite(total_energy) or abs(total_energy) < ZERO_ENERGY_TOL:
...
def calculate_displacement_scale_factor(truss: TrussModel) -> float:
    ...
    try:
        ...
        if len(xs) < 2 or len(ys) < MIN_NODES_SPAN:
            span = 1.0
        else:
...
def generate_plots(
    ...
    try:
        ...
        for element in truss.elements.values():
            if (
                hasattr(element, "node_i")
                and hasattr(element.node_i, "x")
                and hasattr(element, "node_j")
                and hasattr(element.node_j, "x")
            ):
        ...
        for element in truss.elements.values():
            if (
                hasattr(element, "node_i")
                and hasattr(element.node_i, "displacement")
                and hasattr(element, "node_j")
                and hasattr(element.node_j, "displacement")
            ):
        ...
        ax1.set_title(
            f"هندسه خرپا - مقیاس تغییرشکل: {scale_factor:.1f}×",
            fontsize=14,
            fontweight="bold",
        )
    ...
    try:
        if results:
            ...
            filtered_results = [r for r in results if r.get("U", 0) > FILTER_ENERGY_TOL]
            ...
            if len(filtered_results) > 0:
                ...
                wedges, texts, autotexts = ax2.pie(
                    ...
                    labels=labels,
                    colors=colors,
                    autopct=lambda pct: f"{pct:.1f}%" if pct > PIE_MIN_PCT else "",
                    startangle=90,
                    pctdistance=0.85,
                ...
                legend_elements = [
                    Patch(facecolor="#2ecc71", alpha=0.8, label="کشش"),  # type: ignore
                    Patch(facecolor="#e74c3c", alpha=0.8, label="فشار"),  # type: ignore
                ]
