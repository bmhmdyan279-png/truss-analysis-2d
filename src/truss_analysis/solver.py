...
def solve_displacements(truss: TrussModel, K_global, F_global) -> np.ndarray:
    ...
    try:
        if bc_method == "elimination":
            ...
            if zero_rows.size > 0:
                ...
                for idx in range(len(row_sums)):
                    if row_sums[idx] < ZERO_LENGTH_TOLERANCE and abs(F_f[idx]) < ZERO_LENGTH_TOLERANCE:
            ...
            if use_sparse:
                ...
                try:
                    ...
                    logger.info(f"⚠️ خطا در حل با روش پنالتی: {e}. استفاده از روش fallback...")
                ...
                try:
                    ...
                    logger.info(f"⚠️ خطا در حل با روش پنالتی: {e}. استفاده از کمترین مربعات...")
        ...
        logger.info("🔍 پیشنهاد: بررسی کنید که سازه ایستا باشد و تکیه‌گاه‌های کافی داشته باشد.")
...
def validate_energy(results, U_total, truss=None, displacements=None, F=None):
    ...
    if truss is not None:
        # از truss استفاده کن
        has_thermal_effects = any(element.delta_T != 0 or element.delta_L0 != 0 for element in truss.elements.values())
    else:
        # سعی کن از results استخراج کن
        has_thermal_effects = any(r.get("delta_T", 0) != 0 or r.get("delta_L0", 0) != 0 for r in results)
