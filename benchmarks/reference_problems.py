...
class EulerBucklingPinned(ReferenceProblem):
    ...
    def get_reference_value(self) -> float:
        E = 210e9
        i_sec = 5e-5  # not bare `I`: E741, and Element uses I_sec for the same reason
        L = 5.0
        return (np.pi**2 * E * i_sec) / (L**2)  # ~414 kN
...
class CriticalTemperatureBeam(ReferenceProblem):
    ...
    def setup(self) -> dict:
        return {
            ...
            "section": {"type": "HEA200", "A": 0.00538, "I": 3.69e-5},
            "thermal": {
...
class MaterialDegradation600C(ReferenceProblem):
    ...
    def setup(self) -> dict:
        return {"material": "steel_S355", "temperature": 600.0}
...
class ThermalRestraintForce(ReferenceProblem):
    ...
    def setup(self) -> dict:
        return {"length": 3.0, "A": 0.01, "E": 210e9, "alpha": 12e-6, "delta_T": 500.0}
    ...
    for problem in ALL_BENCHMARKS:
        try:
            ...
            if verbose:
                ...
                print(
                    f"  Expected: {benchmark_result.expected_value:.4g} "
                    f"{benchmark_result.units}"
                )
                print(
                    f"  Computed: {benchmark_result.computed_value:.4g} "
                    f"{benchmark_result.units}"
                )
                err_pct = benchmark_result.relative_error * 100
                tol_pct = benchmark_result.tolerance * 100
                print(f"  Error:    {err_pct:.2f}% (tolerance: {tol_pct:.1f}%)")
    ...
    if failed > 0:
        ...
        for r in results:
            if not r.passed:
                ...
                report.append(
                    f"    Error: {r.relative_error * 100:.2f}% "
                    f"(tol: {r.tolerance * 100:.1f}%)"
                )
                if r.notes:
