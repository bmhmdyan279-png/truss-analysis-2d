"""Scenario geometry: exact partition, zero orphans, symmetry.

Negative regressions of the legacy criterion (measured before the fix):
10 orphan members overall and ``pratt_8`` heating only 7 of its 10
left-third members.  The centroid partition must be exact everywhere.
"""
...
# Reference geometric thirds (left/mid/right member counts).
EXPECTED_THIRDS = {
    "warren_4_shallow": (5, 5, 5),
    "warren_6_shallow": (7, 9, 7),
    "warren_8_shallow": (10, 11, 10),
    "pratt_4_shallow": (3, 7, 3),
    "pratt_6_shallow": (6, 9, 6),
    "pratt_8_shallow": (10, 9, 10),
    "howe_4_shallow": (3, 7, 3),
    "howe_6_shallow": (6, 9, 6),
    "howe_8_shallow": (10, 9, 10),
}

# Orphan counts of the LEGACY criteria (must now be zero).
LEGACY_ORPHANS = {
    "warren_4_shallow": 2,
    "warren_6_shallow": 1,
    "warren_8_shallow": 1,
...
def test_third_counts_match_reference(campaign) -> None:
    for cm in campaign:
...
def test_pratt8_local_left_heats_all_ten(campaign) -> None:
    cm = next(c for c in campaign if c.name == "pratt_8_shallow")
...
def test_local_right_scenario_exists_and_matches_partition(campaign) -> None:
    ...
    assert hot == right
    assert cold == ({e.id for e in cm.elements} - right)
