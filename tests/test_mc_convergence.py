"""MC convergence + probabilistic ranking tests (prompt-06 B6/B7)."""
...
def _sample_ci_batch(cm, live, fy, temps, alpha=0.7):
    ...
    for lk, fyk, tk in zip(live, fy, temps):
        ...
        # scale (linearity) and uniform temperature (Lemma 1), so the MC
        # statistic must include the DCR component to depend on the RVs
...
def test_mc_convergence_and_memory(campaign) -> None:
    ...
    specs = proposal_rv_specs(fire_scenario_temperature=400.0)
    ...
    (RESULTS / "mc_convergence.json").write_text(
        json.dumps(
            {
                "seed": SEED,
                "topology": cm.name,
...
def test_probabilistic_ranking_four_steps(campaign) -> None:
    ...
    # the definition quote lives in the module docstring (critics 8/4 of 3.md)
    import truss_analysis.uncertainty.probabilistic_ranking as prmod

    assert "MEAN of CI_i" in (prmod.__doc__ or "")
