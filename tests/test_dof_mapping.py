...
def test_dof_mapping():
    ...
    # Public API: four outputs (K, F_ext, F_mechanical, fixed_dofs)
    K, F_ext, F_mech, fixed = assemble_global_matrices(nodes, [])
    assert 0 in fixed  # DOF 0 constrained
    assert 1 in fixed  # DOF 1 constrained
    assert 2 not in fixed  # DOF 2 free
    assert 3 not in fixed  # DOF 3 free
    assert K.shape == (4, 4)
