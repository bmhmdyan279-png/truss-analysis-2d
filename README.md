...
(`mypy --strict` clean) and covered by a test suite of 689 tests
(93.7 % coverage, gate at 90 %). Results are always SI (m, N, Pa) regardless
...
Current status on this branch: **689 tests passing, 93.74 % coverage**
(`pytest tests/`), `ruff` clean (extended rule set: E, F, I, W, UP, B, SIM,
RUF, PT, N, D), `mypy --strict` clean on all 47 library modules.
...
tests/                                       # 689 tests incl. tests/validation/
